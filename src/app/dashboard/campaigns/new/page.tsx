...
export default function NewCampaignPage() {
  ...
  const label: React.CSSProperties = {
    fontSize: 12, color: s.muted, display: 'block',
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)} required
              placeholder="e.g. Melbourne Restaurant Launch — Food Creators" style={input}
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: s.faint, margin: '5px 0 0' }}>
              This brief is shown to influencers and used by AI to score fit — be specific.
            </p>
          </div>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
              placeholder="Private notes for your team — not visible to influencers. e.g. budget approval status, preferred creators, internal deadlines, deliverable specs…"
              style={{ ...input, resize: 'vertical', lineHeight: 1.6 }}
              onFocus={e => (e.target.style.borderColor = s.primary)} onBlur={e => (e.target.style.borderColor = s.border)} />
            <p style={{ fontSize: 11, color: s.faint, margin: '5px 0 0' }}>
              🔒 Only visible to you — never shown to influencers
            </p>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {NICHE_OPTIONS.map(tag => (
                <button key={tag} type="button" onClick={() => toggleTag(tag, nicheTags, setNiche)} style={tagStyle(nicheTags.includes(tag))}>
                  {tag}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {CONTENT_OPTIONS.map(tag => (
                <button key={tag} type="button" onClick={() => toggleTag(tag, contentTags, setContent)} style={tagStyle(contentTags.includes(tag))}>
                  {tag}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <input value={followerMin} onChange={e => setFollMin(e.target.value)} type="number"
                placeholder="Min (e.g. 10000)" style={input}
                onFocus={e => (e.target.style.borderColor = s.primary)} onBlur={e => (e.target.style.borderColor = s.border)} />
              <input value={followerMax} onChange={e => setFollMax(e.target.value)} type="number"
                placeholder="Max (e.g. 500000)" style={input}
                onFocus={e => (e.target.style.borderColor = s.primary)} onBlur={e => (e.target.style.borderColor = s.border)} />
            </div>
          </div>
          ...
          <div style={{
            ...
            background: publishNow ? s.accent : '#F8FAFC',
            border: `1.5px solid ${publishNow ? s.primary : s.border}`,
            borderRadius: 10, transition: 'all 0.2s',
          }}>
            <div onClick={() => setPublish(v => !v)} style={{
              width: 40, height: 22, borderRadius: 11,
              background: publishNow ? s.primary : s.border,
              position: 'relative', cursor: 'pointer', transition: 'background 0.2s', flexShrink: 0,
            }}>
              <div style={{
                position: 'absolute', top: 3, left: publishNow ? 21 : 3,
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: s.text, margin: 0 }}>
                {publishNow ? 'Publish immediately' : 'Save as draft'}
              </p>
              <p style={{ fontSize: 12, color: s.muted, margin: '2px 0 0' }}>
                {publishNow ? 'Influencers can see and apply right away' : 'Only you can see this until you publish'}
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <a href="/dashboard" style={{
              ...
              border: `1.5px solid ${s.border}`, color: s.muted,
