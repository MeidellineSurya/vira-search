...
type Application = { id: string; status: 'pending' | 'viewed' | 'selected' | 'passed'; applied_at: string; ai_fit_score: number | null; campaigns: { id: string; title: string; description: string; budget_range: string | null; timeline: string | null; niche_tags: string[]; status: string } }
...
export default function ApplicationsPage() {
  ...
  const counts = { pending: applications.filter(a => a.status === 'pending').length, viewed: applications.filter(a => a.status === 'viewed').length, selected: applications.filter(a => a.status === 'selected').length, passed: applications.filter(a => a.status === 'passed').length }
    ...
    <div style={{ minHeight: '100vh', background: '#F8FAFC', fontFamily: '"DM Sans", sans-serif', color: s.text }}>
      <div style={{ borderBottom: `1px solid ${s.border}`, padding: '0 32px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: s.surface, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <nav style={{ display: 'flex', gap: 24, alignItems: 'center', fontSize: 13 }}>
          ...
          <a href="/applications" style={{ color: s.primary, textDecoration: 'none', fontWeight: 600 }}>My applications</a>
          <a href="/profile" style={{ color: s.muted, textDecoration: 'none' }}>Profile</a>
