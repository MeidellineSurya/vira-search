...
function ScrapeQueue() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8FAFC', fontFamily: '"DM Sans", sans-serif', color: s.text }}>
      <div style={{ borderBottom: `1px solid ${s.border}`, padding: '0 32px', height: 56, display: 'flex', alignItems: 'center', background: s.surface }}>
        <span style={{ fontFamily: '"Playfair Display", serif', fontSize: 20, fontWeight: 700, color: s.primary }}>VIRA Admin</span>
        <span style={{ fontSize: 13, color: s.muted, marginLeft: 16 }}>— Scrape Queue</span>
      </div>

      <div style={{ maxWidth: 900, margin: '0 auto', padding: '48px 24px' }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, letterSpacing: '-0.03em', margin: '0 0 6px' }}>Scrape Queue</h1>
        ...
        {!loading && !error && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <h2 style={{ fontSize: 13, fontWeight: 700, color: pending.length > 0 ? '#854D0E' : s.muted, textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>
                  ...
                  <div key={r.id} style={{ background: s.surface, border: `1.5px solid ${s.border}`, borderRadius: 12, padding: '18px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, boxShadow: s.shadow }}>
                    ...
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                      <div style={{ fontSize: 11, color: s.faint, textAlign: 'right', lineHeight: 1.8 }}>
                        ...
                        2. Insert into <code style={{ background: s.accent, padding: '1px 5px', borderRadius: 4, color: s.primary }}>influencers</code> table<br />
                        3. Click Link ↓
                      </div>
                      <button onClick={() => handleLink(r.ig_handle)} disabled={linking === r.ig_handle} style={{ padding: '9px 18px', borderRadius: 8, background: linking === r.ig_handle ? '#F1F5F9' : `linear-gradient(135deg, #0EA5E9, #38BDF8)`, border: 'none', color: linking === r.ig_handle ? s.faint : '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: linking === r.ig_handle ? 'none' : '0 2px 10px rgba(14,165,233,0.3)', whiteSpace: 'nowrap' }}>
