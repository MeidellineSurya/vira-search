...
type Campaign = { id: string; title: string; description: string; budget_range: string | null; timeline: string | null; niche_tags: string[]; content_type_tags: string[]; ideal_follower_min: number | null; ideal_follower_max: number | null }

function fmt(n: number | null) { if (!n) return ''; if (n >= 1_000_000) return (n / 1_000_000).toFixed(1) + 'M'; if (n >= 1_000) return (n / 1_000).toFixed(0) + 'K'; return n.toString() }
  ...
  return (
    <div style={{ minHeight: '100vh', background: s.bg, fontFamily: '"DM Sans", sans-serif', color: s.text }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '48px 24px' }}>
        ...
        {loading ? <p style={{ color: s.faint, fontSize: 14 }}>Loading campaigns…</p>
        : campaigns.length === 0 ? (
                ...
                <div key={campaign.id} style={{ background: s.surface, border: `1.5px solid ${s.border}`, borderRadius: 14, padding: '24px', boxShadow: s.shadow, transition: 'border-color 0.2s' }}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                    ...
                    <button onClick={() => handleApply(campaign.id)} disabled={isApplied || isApplying} style={{
                      padding: '10px 20px', borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: isApplied || isApplying ? 'default' : 'pointer', whiteSpace: 'nowrap', flexShrink: 0, border: 'none', transition: 'all 0.15s', fontFamily: 'inherit',
