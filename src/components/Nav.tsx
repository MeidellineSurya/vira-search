      ...
      <div style={{ padding: '16px 20px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {influencer.industry && (
              <span style={{ fontSize: 11, padding: '2px 10px', borderRadius: 20, background: s.accent, color: s.primaryDark, fontWeight: 600 }}>
                {influencer.industry}
              </span>
            )}
          </div>
          {influencer.bio && (
            <p style={{ fontSize: 13, color: s.muted, margin: '4px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: isExpanded ? 'normal' : 'nowrap', lineHeight: 1.4 }}>
              {influencer.bio}
            </p>
          )}
        </div>
        ...
        <div style={{ padding: '16px 20px', borderTop: `1px solid ${s.border}` }}>
          {!ai.summary && !ai.loadingSummary && (
            <button onClick={generateSummary} style={{
              background: s.accent, border: `1.5px solid ${s.primaryLight}`,
              borderRadius: 8, padding: '9px 18px', color: s.primaryDark,
              fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s',
            }}
              onMouseEnter={e => (e.currentTarget.style.background = '#C7EEFF')}
          ...
          {ai.score !== null && (
            <div style={{ marginTop: 12, padding: 14, background: '#F8FAFC', borderRadius: 10, border: `1.5px solid ${scoreColor(ai.score)}33` }}>
              ...
              {ai.matchReasons.length > 0 && (
                <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {ai.matchReasons.map((r, i) => (
                    <li key={i} style={{ fontSize: 12, color: s.muted, display: 'flex', gap: 6 }}>
                      <span style={{ color: s.primary }}>›</span>{r}
                    </li>
                  ))}
                </ul>
              )}
          ...
          {influencer.instagram_url && (
            <a href={influencer.instagram_url} target="_blank" rel="noopener noreferrer"
              style={{ display: 'inline-block', marginTop: 14, fontSize: 12, color: s.primary, textDecoration: 'none', fontWeight: 600 }}>
              View on Instagram ↗
            </a>
          )}
        </div>
...
export default function SearchPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: s.bg, fontFamily: '"DM Sans", sans-serif', color: s.text }}>
      {/* Nav */}
      <div style={{ borderBottom: `1px solid ${s.border}`, padding: '0 32px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fff', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 1px 0 #E2E8F0' }}>
        ...
        <nav style={{ display: 'flex', gap: 24, alignItems: 'center', fontSize: 13 }}>
          <a href="/search" style={{ color: s.primary, textDecoration: 'none', fontWeight: 600 }}>Search</a>
          <a href="/campaigns" style={{ color: s.muted, textDecoration: 'none' }}>Campaigns</a>
          {role === 'marketer' && <a href="/dashboard" style={{ color: s.muted, textDecoration: 'none' }}>Dashboard</a>}
          {role === 'influencer' && <a href="/applications" style={{ color: s.muted, textDecoration: 'none' }}>My applications</a>}
          {!role && <><a href="/login" style={{ color: s.muted, textDecoration: 'none' }}>Log in</a>
            <a href="/signup" style={{ padding: '7px 16px', borderRadius: 7, background: `linear-gradient(135deg, ${s.primary}, ${s.primaryLight})`, color: '#fff', fontSize: 12, fontWeight: 700, textDecoration: 'none' }}>Sign up</a></>}
        </nav>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 10 }}>
          ...
          <input value={query} onChange={e => handleQueryChange(e.target.value)}
            ...
            style={{ width: '100%', boxSizing: 'border-box', background: '#fff', border: `1.5px solid ${s.border}`, borderRadius: 10, padding: '13px 16px 13px 44px', fontSize: 15, color: s.text, outline: 'none', boxShadow: s.shadow, transition: 'border-color 0.15s', fontFamily: 'inherit' }}
        ...
        <div style={{ marginBottom: 28 }}>
          <button onClick={() => setShowCampaignInput(v => !v)} style={{ background: 'transparent', border: 'none', color: showCampaignInput ? s.primary : s.faint, fontSize: 13, cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
          ...
          {showCampaignInput && (
            <div style={{ marginTop: 10 }}>
              <textarea value={campaignDescription} onChange={e => setCampaign(e.target.value)}
                ...
                rows={3} style={{ width: '100%', boxSizing: 'border-box', background: '#fff', border: `1.5px solid ${s.border}`, borderRadius: 10, padding: '12px 16px', fontSize: 13, color: s.text, resize: 'vertical', outline: 'none', fontFamily: 'inherit', lineHeight: 1.5, transition: 'border-color 0.15s' }}
                onFocus={e => (e.target.style.borderColor = s.primary)} onBlur={e => (e.target.style.borderColor = s.border)} />
        ...
        {!loading && !searched && (
          <div style={{ textAlign: 'center', padding: '48px 0', color: s.faint, fontSize: 14 }}>Type to search, or press Enter to browse all</div>
        )}
        {!loading && results.length > 0 && (
          <>
            <p style={{ fontSize: 12, color: s.faint, marginBottom: 16 }}>
